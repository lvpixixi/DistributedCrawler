package io.renren.modules.spider.dao.impl;
...
import io.renren.modules.spider.config.CrawlerBaseConfig;
...
public class ESClient {
    ...
    @Autowired
    private CrawlerBaseConfig spiderConfig;
    ...
    public Client getClient() {
        if (!spiderConfig.isNeedEs()) {
            LOG.info("已在配置文件中声明不需要ES,如需要ES,请在配置文件中进行配置");
            return null;
        }
        if (client != null) return client;
        LOG.info("正在初始化ElasticSearch客户端," + spiderConfig.getEsHost());
        
        Settings settings = Settings.builder()
        		.put("cluster.name", spiderConfig.getEsClusterName()).build();
        try {
        	client = new PreBuiltTransportClient(settings)
        			.addTransportAddress(new InetSocketTransportAddress(InetAddress.getByName(spiderConfig.getEsHost()), spiderConfig.getEsPort()));
    ...
    public boolean checkType(String index, String type, String mapping) {
        ...
        if (!client.admin().indices().typesExists(new TypesExistsRequest(new String[]{index}, type)).actionGet().isExists()) {
            ...
            try {
                ...
                LOG.info("查找ES mapping配置文件出错, " + e.getLocalizedMessage());
                return false;
            }
    ...
    public boolean checkIndex(String index, String mapping) {
        ...
        if (!client.admin().indices().exists(new IndicesExistsRequest(index)).actionGet().isExists()) {
            ...
            try {
                ...
                LOG.info("查找" + index + "index mapping配置文件出错, " + e.getLocalizedMessage());
                return false;
            }
    ...
    public CrawlerBaseConfig getSpiderConfig() {
        return spiderConfig;
    }

    public ESClient setSpiderConfig(CrawlerBaseConfig spiderConfig) {
