package bscrawler.spider.db.utils;
...
import bscrawler.spider.db.config.PropertiesConfig;
import bscrawler.spider.db.config.PropertiesService;
...
public class DbBuilder {
	...
	public static void main(String[] args) {
		try {
			...
			/*// 返回bean
			News news = DbBuilder.getBean("select * from T_USER where Id=?", News.class, 1);
			System.out.println(news.getId());

			// 返回beanlist
			System.out.println("返回BeanList结果......");
			List<News> beanListResult = DbBuilder.getListBean("select * from T_USER", News.class);
			Iterator<News> iter_beanList = beanListResult.iterator();
			while (iter_beanList.hasNext()) {
				System.out.println(iter_beanList.next().getTitle());
			}*/
