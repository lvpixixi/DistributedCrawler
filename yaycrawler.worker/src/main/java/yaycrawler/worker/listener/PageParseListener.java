...
public class PageParseListener implements IPageParseListener {
    ...
    public void onSuccess(Request request, List<CrawlerRequest> childRequestList) {
        CrawlerResult crawlerResult = new CrawlerResult(CrawlerResult.RESULT_CODE_NEW, DigestUtils.sha1Hex(request.getUrl()), childRequestList, null);
    ...
    public void onError(Request request, String failureInfo) {
        CrawlerResult crawlerResult = new CrawlerResult(CrawlerResult.RESULT_CODE_FAIL, DigestUtils.sha1Hex(request.getUrl()), null, failureInfo);
